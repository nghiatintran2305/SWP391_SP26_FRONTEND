...
import { ClipboardList, FolderKanban, Link2, LogOut, Shield, UserCircle2, Users } from 'lucide-react'
        ...
        <aside className="soft-card h-fit p-4">
          <Link to="/overview" className="mb-6 flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Sync FE with BE</p>
            </div>
          </Link>
          ...
          <nav className="space-y-2">
            ...
            <NavLink to="/projects" className={({ isActive }) => `${linkBase} ${isActive ? linkActive : linkIdle}`}>
              <FolderKanban className="h-4 w-4" /> {roles.includes('STUDENT') ? 'My groups' : 'Projects'}
            </NavLink>
